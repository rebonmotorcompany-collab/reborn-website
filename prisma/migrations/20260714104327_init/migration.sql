...
CREATE TABLE `User` (
    ...
    INDEX `User_dealerId_idx`(`dealerId`),
    PRIMARY KEY (`id`)
...
CREATE TABLE `Media` (
    ...
    `type` ENUM('IMAGE', 'VIDEO', 'PDF', 'DOCUMENT', 'OTHER') NOT NULL,
...
CREATE TABLE `ActivityLog` (
    ...
    `action` ENUM('CREATE', 'UPDATE', 'DELETE', 'LOGIN', 'LOGOUT', 'PASSWORD_RESET', 'PERMISSION_DENIED', 'OTHER') NOT NULL,
...
-- AddForeignKey
ALTER TABLE `Dealer` ADD CONSTRAINT `Dealer_companyId_fkey` FOREIGN KEY (`companyId`) REFERENCES `Company`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
