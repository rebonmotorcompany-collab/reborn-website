...
export default function ProductForm({ initialData, isEditing }: ProductFormProps) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // Auto-generate slug from name if not manually edited yet
    if (name === 'name' && !isEditing && (!formData.slug || formData.slug === generateSlug(formData.name))) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEditing 
        ? `/api/cms/products?id=${initialData.id}` 
        : '/api/cms/products'
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <Link href="/dashboard/products" className="p-2 rounded-full hover:bg-neutral-200 dark:hover:bg-neutral-800 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="bg-white dark:bg-neutral-950 p-6 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm space-y-5">
            <h2 className="text-lg font-semibold text-neutral-900 dark:text-white border-b border-neutral-100 dark:border-neutral-800 pb-3">General Information</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-xs text-neutral-500 mt-1">The URL-friendly name. E.g., e-volt-x</p>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Full Specifications / Content</label>
                <textarea
                  name="content"
                  required
                  ...
                  placeholder="JSON string or Markdown describing specs..."
                />
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-neutral-950 p-6 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm space-y-5">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Product Type</label>
                <select
                  name="type"
                  value={formData.type}
                  onChange={handleChange}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="w-full px-4 py-2 border border-neutral-300 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-900 text-neutral-900 dark:text-white focus:ring-2 focus:ring-red-500 outline-none"
                  placeholder="e.g. Rs 240,000"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Featured Image</label>
                <MediaPicker
                  value={formData.image}
                  onChange={url => setFormData(prev => ({ ...prev, image: url }))}
                  placeholder="Choose featured image from media library..."
                />
              </div>
